...
- **Hashing rule:** hash at the **message level** (normalized concat of all text blocks), **not** per-block. Claude's provider mapper is **one internal message → one wire message**; the only coalescing is *within* a single message's `content` array (a real capture showed one user message as `content:["text","text"]`). Since Bonsai prunes whole-message ranges and the mapping is 1:1, message-level canonical hashing is the correct granularity.
