...
- **Semantic anchor evidence:** target-bundle evidence for patch anchors must show why each chosen location in the pinned Claude Code target is the correct behavior seam for the required patch. Reviewers must reject artifact or e2e evidence that relies on syntax matches, candidate counts, sentinel insertion, or happy-path/synthetic fixtures without explaining the real host behavior and rejecting plausible wrong locations.
...
Contract B tests and release-gate evidence MUST prove more than helper mechanics. They must include semantic anchor analysis for the pinned target: what host behavior the chosen code controls, why that behavior is the required seam, why plausible nearby candidates are wrong, how ambiguous/no-match cases fail closed, and what pinned-target behavior proves the patch works. `verifySentinel` is mandatory but insufficient by itself because it proves insertion happened at the selected location, not that the selected anchor was semantically correct. Synthetic fixtures may test helper behavior, but they are not acceptance evidence for Claude Code anchor correctness. Do not weaken `minScore`/`minMargin` or bypass `AnchorAmbiguousError` to make the pinned target pass.
