...
- Anchor-evidence bar: tests and review evidence must prove all three anchors target the actual Claude Code token/gauge, attachment, and model-visible reminder behavior required by this story. Synthetic fixtures may test helper mechanics, but happy-path fixtures and sentinel-only checks are not acceptance evidence. Include evidence that plausible unrelated token, attachment, and reminder candidates are wrong, ambiguous plausible anchors fail closed, no-match fails closed for each anchor, and the chosen anchors change the required model-facing behavior; do not weaken `minScore`/`minMargin` or fail-closed behavior to pass. Missing semantic anchor analysis is a HIGH/CRITICAL review finding depending on release impact.
